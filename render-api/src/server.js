...
app.post('/api/requests', rateLimit({ windowMs: 60 * 60 * 1000, limit: 5, standardHeaders: true, legacyHeaders: false }), async (req, res, next) => {
  try {
    ...
    res.status(201).json({ message: 'You have successfully joined the Zoom queue. Aapki baari aane par hum aapko Add karenge.' })
...
app.delete('/api/admin/requests/:id', adminOnly, async (req, res, next) => { try { const request = await SupportRequest.findByIdAndDelete(req.params.id); if (!request) return res.status(404).json({ message: 'Request not found.' }); res.json({ message: 'Deleted.' }) } catch (error) { next(error) } })
